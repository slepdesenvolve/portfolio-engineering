...
export const AutoDocVisualizer: React.FC = () => {
  ...
  // --- MOTOR DE ANÁLISE MODULAR (OTIMIZADO COM LAZY LOADING) ---
  const analyzeCode = async () => {
    ...
    try {
      /* ============================================================
      BACKUP: CHAMADA ESTÁTICA ANTERIOR (Remover import do topo)
      ...
      */
      ...
      // NOVO: CARREGAMENTO DINÂMICO (Lazy Loading)
      addLog("Carregando módulos de análise sob demanda via AST Engine...");
      const { LinterEngine } = await import('../../core/linter/engine');
      ...
      addLog(`ERRO_CRÍTICO: Falha no motor de análise dinâmica.`);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-bg-card border border-border-main rounded-2xl overflow-hidden shadow-2xl">
      {/* 
        ...
        ========================================================================
      */}
      <div className="bg-bg-base border-b border-border-main p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-3 h-3 rounded-full bg-red-500/50" />
          <div className="w-3 h-3 rounded-full bg-amber-500/50" />
          <div className="w-3 h-3 rounded-full bg-emerald-500/50" />
          <span className="mono text-xs text-text-muted ml-4 uppercase tracking-widest">
        ...
        <div className="flex items-center gap-4">
          {showResults && (
            <div className="flex items-center gap-2 px-3 py-1 bg-bg-base border border-border-main rounded-full relative group">
              <span className="mono text-[9px] text-text-muted uppercase">
                Global_Score:
              </span>
              <span
              ...
              {/* Tooltip */}
              <div className="absolute top-full right-0 mt-2 w-64 p-4 bg-bg-card border border-border-main rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 pointer-events-none">
                <h4 className="mono text-[10px] font-bold text-secondary uppercase mb-3 border-b border-border-main pb-2">
                ...
                <div className="mt-3 pt-2 border-t border-border-main">
                  <p className="text-[8px] text-text-muted italic leading-tight">
                    * O score final é a média aritmética de todas as funções
                    analisadas.
                  </p>
                </div>
              </div>
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg mono text-[10px] font-bold text-text-muted hover:text-red-400 transition-colors border border-border-main hover:border-red-400/30"
          >
            RESET
          </button>
          <button
            ...
            className={`flex items-center gap-2 px-4 py-1.5 rounded-lg mono text-xs font-bold transition-all ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 h-[550px]">
        {/* 
          ...
          ========================================================================
        */}
        <div className="border-r border-border-main flex flex-col min-h-0 bg-bg-base/20">
          <div className="bg-bg-base/50 p-2 border-b border-border-main flex items-center justify-between">
            ...
            <span className="mono text-[9px] text-text-muted/50 italic">
        ...
        {/* 
          ...
          ========================================================================
        */}
        <div className="bg-bg-base/30 flex flex-col relative min-h-0">
                ...
                <div className="p-4 space-y-2">
                  ...
                  {!isAnalyzing && logs.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-center opacity-20 mt-20">
                      ...
                      <p className="mono text-xs uppercase tracking-tighter">
                        Aguardando entrada de código...
                      </p>
                    </div>
                  )}
                </div>
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <h4 className="mono text-sm font-bold text-primary">
                              {res.name}
                            </h4>
                          </div>
                          <span
                        ...
                        {res.warnings.length > 0 && (
                          <div className="mb-4 space-y-1">
                            {res.warnings.map((w, wi) => (
                              <div
                                key={wi}
                                className="flex items-center gap-2 text-[10px] text-amber-500/80 mono"
                              >
                        ...
                        <div className="space-y-2">
                          ...
                          {res.params.map((p, pi) => (
                            ...
                            >
                              ...
                              <span
                                className={`text-accent ${p.type === "any" ? "text-red-400 font-bold underline decoration-dotted" : ""}`}
                              >
                                {p.type}
                              </span>
                            </div>
                        ...
                        <div className="mt-4 pt-2 border-t border-border-main flex justify-between items-center">
                          ...
                          <span
                            className={`text-[10px] mono font-bold ${res.type.includes("any") ? "text-red-400" : "text-emerald-400"}`}
                          >
                            {res.type}
                          </span>
                        </div>
      ...
      */}
      <div className="bg-bg-base border-t border-border-main p-3 flex items-center justify-between px-6">
        <div className="flex items-center gap-4">
          ...
          <span className="mono text-[9px] text-text-muted uppercase">
            Mode: Real-Time_Parser
          </span>
        </div>
        <div className="flex items-center gap-2 opacity-50">
          <AlertCircle className="w-3 h-3 text-text-muted" />
          <span className="mono text-[9px] text-text-muted uppercase italic">
            Strict_Mode: Enabled
          </span>
        </div>
      </div>
